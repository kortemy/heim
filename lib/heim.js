class Heim {
  ...
  async sortFavorites (element) {
    let favorites = this.storage.getFavorites()

    element.querySelectorAll('.favorite').forEach(async (el, i) => {
      let data = el.getAttribute('data-favorite')
      favorites[data].index = i
    })
  ...
  async initFavorites () {
    ...
    let favorites = this.storage.getFavorites()
    let sorted = Object.values(favorites).sort((a, b) => a.index - b.index)
    ...
    await Promise.all(sorted.map(async fv => {
      if (fv.id) {
    ...
    return new Promise((resolve, reject) => {
      chrome.bookmarks.getTree(async result => {
        if (!result || result.length === 0) {
          document.querySelector('#bookmarks .empty').style.display = 'block'
          return
        }
  ...
  async initDevices () {
    return new Promise((resolve, reject) => {
      chrome.sessions.getDevices({}, async result => {
        if (!result || result.length === 0) {
          document.querySelector('#devices .empty').style.display = 'block'
          return
        }
  ...
  async initApps () {
    return new Promise((resolve, reject) => {
      chrome.management.getAll(async result => {
        if (!result || result.length === 0) {
          document.querySelector('#apps .empty').style.display = 'block'
          return
        }
        let apps = result.filter(app => ['packaged_app', 'hosted_app', 'legacy_packaged_app'].includes(app.type)).sort((a, b) => {
